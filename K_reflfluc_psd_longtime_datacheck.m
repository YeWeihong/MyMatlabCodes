function K_reflfluc_psd_longtime_datacheck(shot,time1,time2,card_name)
...
time_span=[time1 time2];
 %data_path='X:\reflfluc\Refl_Fluc_2019\O_20-40';
 data_path='X:\reflfluc\Refl_Fluc_2019';
fs=2*10^6;
...
[t_3D,f_3D,PSD1,PSD2,PSD3,PSD4]=K_reflfluc_3D(shot,time_span,card_name,data_path,fs,time_delay,judge); 


figure('name',card_name);
...
imagesc(t_3D,f_3D,log10(PSD1));
      ...
      set(gca,'YDir','normal')
      hold on
      title([shot ' Refl.@' card_name 'frequency f=' num2str(freq_inj(1)) 'GHz'])
...
imagesc(t_3D,f_3D,log10(PSD2));
      ...
      set(gca,'YDir','normal')
      hold on
      title([shot ' Refl.@' card_name 'frequency f=' num2str(freq_inj(2)) 'GHz'])
...
imagesc(t_3D,f_3D,log10(PSD3));
      ...
      set(gca,'YDir','normal')
      hold on
      title([shot ' Refl.@' card_name 'frequency f=' num2str(freq_inj(3)) 'GHz'])
...
imagesc(t_3D,f_3D,log10(PSD4));
      ...
      set(gca,'YDir','normal')
      hold on
      title([shot ' Refl.@' card_name 'frequency f=' num2str(freq_inj(4)) 'GHz'])
      linkaxes(ax) 
        ylim([0 100])
...
%     judge=1; %% judge=1: original signal judge=2: amplitude signal 
    ...
    figure('name',card_name)
    ax(1)=subplot(411)
    plot(t_signal,real(S1))
    hold on
    plot(t_signal,imag(S1),'r')
    ax(2)=subplot(412)
    plot(t_signal,real(S2))
    hold on
    plot(t_signal,imag(S2),'r')
    ax(3)=subplot(413)
    plot(t_signal,real(S3))
    hold on
    plot(t_signal,imag(S3),'r')
    ax(4)=subplot(414)
    plot(t_signal,real(S4))
    hold on
    plot(t_signal,imag(S4),'r')
    linkaxes(ax,'x') 
